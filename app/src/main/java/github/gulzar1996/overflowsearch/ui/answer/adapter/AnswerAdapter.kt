...
class AnswerAdapter(val retryCallback: () -> Unit)
    : PagedListAdapter<Answer, RecyclerView.ViewHolder>(AnswerDiffCallback) {

    private var networkState: NetworkState? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            R.layout.item_answer -> AnswerViewHolder.create(parent)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            R.layout.item_answer -> (holder as AnswerViewHolder).bindTo(getItem(position))
